...
angularGoogleAuth2.controller("S_SignInController", ["$scope", "S_PersonalData", "S_GeneralData", "$location", "$http", "$sce", function ($scope, S_PersonalData, S_GeneralData, $location, $http, $sce) {
    ...
    /* Madeira JArdas Register on Stardog*/
    function persistUserIfNotPersisted($name, $email, $picture, $id) {
        var parameter = JSON.stringify({name: $name, email: $email, picture: $picture, id: $id});
        var url = "http://" + $scope.wsUrl + "/ws/user/";
