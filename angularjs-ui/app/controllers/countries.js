...
// Home controller
mmodule.controller('CountriesCtrl', ['$scope', 'S_PersonalData', '$location', '$document', '$http', '$sce', '$window', 'NgMap', '$interval', '$q','toastr', function ($scope, S_PersonalData, $location, $document, $http, $sce, $window, NgMap, $interval, $q,toastr) {
    ...
    $scope.pesquisarCountry = function (pesquisa) {
        var deferred = $q.defer();
        var  query = "http://" + $scope.wsUrl + "/ws/country/name/"+pesquisa;
    ...
    $scope.pesquisar = function (pesquisa) {
        ...
        if (typeof pesquisa !== "undefined" && pesquisa[0] == '#') {
            ...
            if (categoriesSelected.length !== 0) {
                if (pesquisa !== "")
                    url = "http://" + $scope.wsUrl + "/ws/movie/country/" + pesquisa + "/category/";
                else
                ...
                url = "http://" + $scope.wsUrl + "/ws/movie/country/" + pesquisa;
            latestP = url;
