...
// Home controller
mmodule.controller('ActorsCtrl', ['$scope', 'S_PersonalData', '$location', '$document', '$http', '$sce', '$window', 'NgMap', '$interval', '$q','toastr', function ($scope, S_PersonalData, $location, $document, $http, $sce, $window, NgMap, $interval, $q,toastr) {
    ...
    $scope.pesquisar = function (pesquisa) {
        ...
        if (typeof pesquisa !== "undefined" && pesquisa[0] == '#') {
            ...
            if (categoriesSelected.length !== 0) {
                if (pesquisa !== "")
                    url = "http://" + $scope.wsUrl + "/ws/movie/actor/" + pesquisa + "/category/";
                else
                ...
                url = "http://" + $scope.wsUrl + "/ws/movie/actor/" + pesquisa;
            latestP = url;
    ...
    $scope.pesquisarActor = function (pesquisa) {
        var deferred = $q.defer();
        var  query = "http://" + $scope.wsUrl + "/ws/actor/name/"+pesquisa;
        $http.jsonp($sce.trustAsResourceUrl(query)).then(
            function onSuccess(data) {
